...
public class ShowItemsUser extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("makeBid") != null) {
			...
			if (testBiddingError(startBid, currentBid, bidInc, newBidValue)) {
			...
			Bid bid = new Bid((Integer) session.getAttribute("userId"),
					Integer.parseInt(request.getParameter("itemId")), Double.parseDouble(newBidValue));
			bidDAO.addBid(bid);
		}
		...
		if (request.getParameter("buyItNow") != null) {
			...
			Bid bid = new Bid((Integer) session.getAttribute("userId"),
					Integer.parseInt(request.getParameter("itemId")), Double.parseDouble(startBid));
	...
	public boolean testBiddingError(String startBid, String currentBid, String bidInc, String newBid) {
		try {
			if ((Double.parseDouble(newBid) < (Double.parseDouble(currentBid) + Double.parseDouble(bidInc))
